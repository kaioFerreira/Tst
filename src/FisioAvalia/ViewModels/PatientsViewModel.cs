...
namespace FisioAvalia.ViewModels
{
    ...
    {
        readonly IList<PatientsModel> source;
        readonly IList<PatientsModel> sourceActive;
        readonly IList<PatientsModel> sourceArchived;
        PatientsModel selectedPatients;
        int selectionCount = 1;
        int selectionCountActive = 1;
        int selectionCountArchived = 1;

        public ObservableCollection<PatientsModel> Patients { get; private set; }

        public IList<PatientsModel> EmptyPatients { get; private set; }


        public PatientsModel SelectedPatients
        {
            get
            {
        ...
        public string SelectedPatientsMessageActive { get; private set; }

        public ICommand FilterCommand => new Command<string>(FilterItems);
        public ICommand PatientsSelectionChangedCommand => new Command(PatientsSelectionChanged);
        public ICommand PatientsSelectionChangedActiveCommand => new Command(PatientsSelectionChangedActive);

        public PatientsViewModel(string Tipo)
        {
            if (Tipo == "Todos")
            {
                source = new List<PatientsModel>();
                CreatePatientsCollectionAll();
                selectedPatients = Patients.Skip(3).FirstOrDefault();
                PatientsSelectionChanged();
            }
            else if (Tipo == "Ativos")
            {
                sourceActive = new List<PatientsModel>();
                CreatePatientsCollectionActive();
                selectedPatients = Patients.Skip(0).FirstOrDefault();
                PatientsSelectionChangedActive();
            }
            else //Archived
            {
                sourceArchived = new List<PatientsModel>();
                CreatePatientsCollectionArchived();
                selectedPatients = Patients.Skip(0).FirstOrDefault();
                PatientsSelectionChangedArchived();
            }
        }

        void CreatePatientsCollectionActive()
        {
            sourceActive.Add(new PatientsModel
            {
                Name = "Kaio Ferreira",
                Location = "Goiania - GO",
                Details = "Exame Fisico",
                ImageUrl = "https://scontent.fbsb11-1.fna.fbcdn.net/v/t1.0-1/p160x160/37723352_1826622114081322_5233948271910584320_n.jpg?_nc_cat=101&_nc_eui2=AeEigxZ432TfVZMQ5NIeagzukVqz1b1_2ePuCEf7LITf7gFZi9pvh40j9tygvrbWU9qmB33bd6nHl6EG48LUUWaTLKbAE4Pz2imMwI1cIqU2ug&_nc_oc=AQmmvNNohleDeLZ4DpMOVfWyU1RGf_UWkpoT1-5Ejj1M543471ShfZZwKK9WuDwpi5c&_nc_ht=scontent.fbsb11-1.fna&oh=fabab75ad801804210a642c4d4965cc7&oe=5D7E27DA"
            });
            ...
            Patients = new ObservableCollection<PatientsModel>(sourceActive);
        }

        void CreatePatientsCollectionArchived()
        {
            sourceArchived.Add(new PatientsModel
            {
                Name = "Macaco",
                Location = "Australia - US",
                Details = "Dor de dente",
            ...
            Patients = new ObservableCollection<PatientsModel>(sourceArchived);
        }

        void CreatePatientsCollectionAll()
        {
            source.Add(new PatientsModel
            {
                Name = "Baboon",
            ...
            Patients = new ObservableCollection<PatientsModel>(source);
        }


        void FilterItems(string filter)
        {
            var filteredItems = source.Where(Patients => Patients.Name.ToLower().Contains(filter.ToLower())).ToList();
        ...
        {
            SelectedPatientsMessage = $"Selection {selectionCount}: {selectedPatients.Name}";
            OnPropertyChanged("SelectedPatientsMessage");
            selectionCount++;
        }

        void PatientsSelectionChangedActive()
        {
            SelectedPatientsMessage = $"Selection {selectionCountActive}: {selectedPatients.Name}";
            OnPropertyChanged("SelectedPatientsMessage");
            selectionCountActive++;
        }

        void PatientsSelectionChangedArchived()
        {
            SelectedPatientsMessage = $"Selection {selectionCountArchived}: {selectedPatients.Name}";
            OnPropertyChanged("SelectedPatientsMessage");
            selectionCountActive++;
        }
